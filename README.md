...
```mermaid
flowchart TB
  USER([사용자]) --> APP["Streamlit (app.py)"]

  APP --> T2S["Text2SQL Agent<br/>text2sql.py"]
  APP --> OTHER["Agent · RAG · 대화 · 번역 · 이미지<br/>chat.py · langgraph_agent.py"]

  subgraph CORE["Text2SQL 핵심 흐름"]
    direction LR
    S1["① example query 검색"]
    S2["② schema linking"]
    S3["③ SQL 생성 · 검증 · 실행"]
  ...
  OTHER --> LG["LangGraph agent ⇄ action"]
  LG --> MCP["MCP 도구<br/>text2sql · RAG · 기타"]

  subgraph BEDROCK["Amazon Bedrock"]
    LLM["ChatBedrock / Runtime"]
